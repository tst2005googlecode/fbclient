...
function encode_schar(v)
	assert(isschar(v),'8bit signed integer expected')
	return struct.pack('BB',1,v)
end

function encode_bool(v)
...
function encode_none(v)
	assert(v==true,'an option that takes no arguments must be given the value true')
	return ''
end

-- isc_dpb_shutdown is the only user of this encoder
function encode_bitmask(bits)
	return function(v)
		local n = 0
...
function encode(pb_type, pb_header, opts, codes, encoders)
	local s = pb_header
	if opts then
