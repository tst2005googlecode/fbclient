--[[
	...
	USAGE:
	- use it with isc_dsql_sql_info() to encode the info request and decode the info result.
	- see info_request_codes table below for what you can request and info_reply_codes for the
	structure and meaning of the results.
...
local decoders = {
	...
	isc_info_sql_stmt_type = function(s) -- custom decorer made because you can get one or two stmt. types
		...
		for i=1,#s do
			...
			ret[#ret+1] = asserts(stmt_code_lookup[code], 'unknown stmt_code %d returned by server',code)
		end
		return ret
	end,
	...
	isc_info_sql_records = function(s)
		...
		for i=1,4 do
			...
			local cn = asserts(records_cluster_code_lookup[c],'unknown records cluster code %d',c)
			assert(sz==INT_SIZE)
			ret[cn] = n
		end
		return ret
