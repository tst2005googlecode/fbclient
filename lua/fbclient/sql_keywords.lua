...
--this is a list that was given to me in firebird-devel mailing list
keywords['SQL-Standard'] = index{
	...
	'DOUBLE', 'DROP', 'DYNAMIC', 'EACH', 'ELEMENT', 'ELSE', 'END', 'END-EXEC', 'ESCAPE',
...
--the keyword list we're basing the modifications on.
...
}, copy(keywords['SQL-Standard'], index{
...
if __UNITTESTING then
	--print a csv of all the keyword modes
	local all = {}
	for name, list in pairs(keywords) do
		copy(list, all)
	end
...
return function(mode, patt)
	if keywords[mode] then
		return keywords[mode]
	else
		local t = {}
		cache[mode] = t
		for mode, keywords in pairs(keywords) do
			if mode:find(patt) then
				copy(keywords, t)
			end
		end
		return t
