--[=[
	...
	attach(fbapi, sv, svcname, spb_options_t) -> svh; for TCP/IP, svcname has the form "<hostname>:service_mgr"
	detach(fbapi, sv, svh)
	start(fbapi, sv, svh, action, [action_options_t])
	query(fbapi, sv, svh, request_options_t, [spb_options_t], [info_buf, info_buf_len]) -> info_t, result_buffer, result_buffer_size

	USAGE/NOTES:
	- you might wanna use service_class.lua, which is a higher level interface based on this wrapper.
	- checkout test_service_wrapper.lua and service_class.lua for complete coverage of this wrapper.
	...
	TODO:
	- isc_spb_sts_table: must be a list of table names, but it's a bitmask instead (wtf!?!?)
...
function detach(fbapi, sv, svh)
	fbtry(fbapi, sv, 'isc_service_detach', svh)
end
...
--actions: they represent tasks to be started over a Service Manager attachment.
local action_codes = {
...
local isc_spb_prp_shutdown_mode_enum = {
	...
	isc_spb_prp_sm_full   = 3, --full shutdown, disabling new attachments
}
...
local isc_info_svc_db_stats_bits = {
	...
	isc_spb_sts_table           = 0x40, --undocumented
...
function start(fbapi, sv, svh, action, opts)
	...
	fbtry(fbapi, sv, 'isc_service_start', svh, nil, #s, s)
end

--Query SPB: used for parametrizing a query to an Service Manager attachment.
...
local info_codes = {
	isc_info_svc_svr_db_info		= 50,	--get number of attachments and databases
	...
	isc_info_svc_running			= 67,	--get check to see if a service is running on an attachment
...
function query(fbapi, sv, svh, request_opts, sbp_opts, buf, buf_size)
